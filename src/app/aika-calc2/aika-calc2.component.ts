...
import { SkillSelectPopupComponent } from "../skill-select-popup/skill-select-popup.component";

@Component({
    selector: 'app-aika-calc2',
    standalone: true,
    imports: [
        CommonModule,
        FormsModule,
        SkillSelectPopupComponent
    ],
    templateUrl: './aika-calc2.component.html',
    styleUrl: './aika-calc2.component.scss'
})
export class AikaCalc2Component {
...
射擊/近戰+100`
    ...
    ngOnInit(): void {
        ...
        this.openSkillSelector(this.character.skills);
    ...
    public openSkillSelector(skills: Skill[]): void {
        this.service.skillSelector.skills = [];
        for (let i = 0; i < 50; i++) {
            this.service.skillSelector.skills.push({
                name: 'test'+i,
                description: 'test_test_test'+i,
                buffs: [new Buff(AttrTypeId.HP, 0.1)],
                subSkills: [],
            });
        }
        this.service.skillSelector.onSelect = (skill: Skill) => {
            skills.push(skill);
            this.service.skillSelector.show = false;
        };
        this.service.skillSelector.show = true;
    }
    removeSkill(skills: Skill[], skill: Skill): void {
